	...
	if strings.Count(r.URL.Path[1:], "/") == 1 {
		...
		if strings.HasSuffix(path[1], ".rss") {
			...
			for _, _ = range r1.Urls {
				...
				if feed.PubDate != "" {
					rootFeed.PubDate = feed.PubDate
				}
				for _, v := range feed.Entries {
					entries = append(entries, v)
				}
			}
			rootFeed.Title = "俺のフィード"
			rootFeed.Link = r.URL.String()
			rootFeed.Entries = entries
			b, err := xml.Marshal(util.FeedToRss(rootFeed))
			if err != nil {
